...
def _run3_nanoV12_ak4puppi_jet_id_correctionlib(jets, payload_path, correction_key):
...
def run3_nanoV12_ak4puppi_jet_id(jets, year, working_point="tight"):
    ...
    if not correctionlib_missing:
        return _run3_nanoV12_ak4puppi_jet_id_correctionlib(
            jets, payload_path, correction_key
        )

    missing_non_multiplicity = [
        field for field in correctionlib_missing
        if field not in _RUN3_NANOV12_JET_ID_MULTIPLICITY_FIELDS
    ]
    if correctionlib_missing and not missing_non_multiplicity:
