...
def _broadcast_flat_event_field(values, target_length, name):
    """Validate flat-mode event/lumi inputs for per-muon RandomSmearing."""
    values_arr = np.asarray(values)
    if values_arr.ndim == 0:
        return np.full(target_length, values_arr.item())
    values_arr = values_arr.reshape(-1)
    if len(values_arr) != target_length:
        raise ValueError(
            f"{name} must be scalar or have one entry per flat muon "
            f"({target_length}); got {len(values_arr)}."
        )
    return values_arr
...
def get_rndm(eta, phi, nL, evtNr, lumiNr, cset, nested=False):
    ...
    if nested:
        ...
        # Local topcoffea patch: upstream 8d0de18 left flat-mode
        # evtNr_f/lumiNr_f undefined after moving RandomSmearing to
        # correctionlib.  In flat mode these are one value per muon, with
        # scalar broadcast allowed for single-event probes.
        target_length = len(np.atleast_1d(np.asarray(eta_f)))
...
def pt_resol(pt, eta, phi, nL, evtNr, lumiNr, cset, nested=False, low_pt_threshold = 26):
    ...
    """
    # Local topcoffea patch: correctionlib RandomSmearing replaced the
    # upstream rnd_gen selector, so do not forward the stale argument.
