...
`topcoffea.modules.compat` now ships the shims previously hosted inside
`topeft/topeft/compat`. Projects running on Python 3.9 should invoke
`compat.ensure_histEFT_py39_compat()` before importing `topcoffea.modules.histEFT`
to patch PEP 604 union annotations, and use `compat.ensure_hist_utils()` when
`topcoffea.modules.hist_utils` needs to be available despite missing optional
dependencies.
