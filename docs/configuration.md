...
now rely exclusively on eager Awkward 2.x arrays: jet/MET corrections no longer
accept or require caches, and all systematic branches are built from materialised
arrays that already share the per-event jagged structure before they are zipped
into records.  This cache-free workflow mirrors coffea ≥0.7 expectations and
avoids the awkward virtual plumbing that triggered broadcast issues with newer
Awkward releases.【F:topcoffea/modules/CorrectedJetsFactory.py†L12-L199】【F:topcoffea/modules/CorrectedMETFactory.py†L35-L59】
